...
<script type="text/javascript">
    ...
    const data = {
        ...
        datasets: [{
          label: 'User Marks',
          data: [{{ App\Models\UserTest::where('user_id',$user->id)->where('level','1')->where('role_specific_id',$role_specific->id)->pluck('mark')->toArray()[0] ?? 0}},
          {{ App\Models\UserTest::where('user_id',$user->id)->where('level','2')->where('role_specific_id',$role_specific->id)->pluck('mark')->toArray()[0] ?? 0}},
          {{ App\Models\UserTest::where('user_id',$user->id)->where('level','3')->where('role_specific_id',$role_specific->id)->pluck('mark')->toArray()[0] ?? 0}},
          {{ App\Models\UserTest::where('user_id',$user->id)->where('level','4')->where('role_specific_id',$role_specific->id)->pluck('mark')->toArray()[0] ?? 0}}],
          fill: true,
